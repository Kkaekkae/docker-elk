...
function ensure_role {
	...
	if [[ "$http_code" != "200" ]]; then
		suberr "Role creation failed for '$name' (HTTP $http_code):"
		suberr "$body"
		return 1
	fi
...
function wait_for_cluster_ready {
  ...
  for _ in $(seq 1 30); do
    status=$(curl -s -u elastic:"${ELASTIC_PASSWORD}" "http://${elasticsearch_host}:9200/_cluster/health" | grep -o '"status":"[^"]*"' | cut -d':' -f2 | tr -d '"')
